...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated tachacoin to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (fPreSegWit) {
        result.push_back(Pair("sizelimit", (int64_t)dgpMaxBlockSize)); // tachacoin
    } else {
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in TACHACOIN)\n"
