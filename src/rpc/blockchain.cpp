...
{
    if (pindexBestHeader->nHeight >= Params().GetConsensus().nLastPOWBlock)//tonitachacoin edit
        return 0;
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // tachacoin
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // tachacoin
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // tachacoin
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // tachacoin
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress tachacoinSenderAddress(request.params[2].get_str());
        if(tachacoinSenderAddress.IsValid()){
            CKeyID keyid;
            tachacoinSenderAddress.GetKeyID(keyid);
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "callcontract",           &callcontract,           true,  {"address","data"} }, // tachacoin
