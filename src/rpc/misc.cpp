...
UniValue getdgpinfo(const JSONRPCRequest& request)
{
    ...
    obj.push_back(Pair("maxblocksize", (uint64_t)tachacoinDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (uint64_t)tachacoinDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (uint64_t)tachacoinDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total tachacoin balance of the wallet\n"
            "  \"stake\": xxxxxxx,           (numeric) the total tachacoin stake balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The tachacoin address to validate\n"
            "\nResult:\n"
            "{\n"
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are tachacoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) tachacoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The tachacoin address to use for the signature.\n"
