...
{
    CBitcoinAddress tachacoinAddress(strTachacoinAddress);
    if(tachacoinAddress.IsValid()){
        CKeyID keyid;
        tachacoinAddress.GetKeyID(keyid);
...
bool ToTachacoinAddress(const std::string& strHash160, std::string& strTachacoinAddress)
{
    ...
    CKeyID keyid(key);
    CBitcoinAddress tachacoinAddress;
    tachacoinAddress.Set(keyid);
    if(tachacoinAddress.IsValid()){
        strTachacoinAddress = tachacoinAddress.ToString();
        return true;
    }
    return false;
}
