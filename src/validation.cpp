...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // tachacoin
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            TachacoinTxConverter converter(tx, NULL);
            ...
            for(TachacoinTransaction tachacoinTransaction : tachacoinTransactions){
                ...
                if(tachacoinTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s TACHACOIN additional fees, %d delta bytes\n",
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    ByteCodeExec exec(block, std::vector<TachacoinTransaction>(1, callTransaction), blockGasLimit);
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // tachacoin
    TachacoinDGP tachacoinDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setTachacoinSchedule(tachacoinDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = tachacoinDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = tachacoinDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = tachacoinDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // tachacoin
...
///////////////////////////////////////////////////////////////////////////////////////// tachacoin
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            TachacoinTxConverter convert(tx, &view, &block.vtx);
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertTachacoinTX.first, blockGasLimit);
