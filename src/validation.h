...
extern bool fAddressIndex; // tachacoin
...
public:
    ...
    TachacoinTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<TachacoinTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
